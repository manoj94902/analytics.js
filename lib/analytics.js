...
Analytics.prototype.identify = function (id, traits, options, fn) {
  ...
  var msg = this.normalize({
    options: defaults({ page: pageContext() }, options),
    traits: user.traits(),
    userId: user.id(),
  });
...
Analytics.prototype.page = function (category, name, properties, options, fn) {
  ...
  var page = pageContext(properties);
  options = options || {};
  options.page = page;

  properties = defaults(clone(page), properties);
...
/**
 * Accepts a page call's `.properties` object and transforms it into a
 * spec-compliant `context.page` object.
 *
 * @param {Object} properties
 * @return {Object}
 */

function pageContext(properties){
  properties = properties || {};
  ...
  var reservedProperties = {
    ...
    url: canonicalUrl(properties.search || location.search)
  };

  return defaults(
    pick(keys(reservedProperties), properties),
    reservedProperties
  );
}
